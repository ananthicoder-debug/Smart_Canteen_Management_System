...
router.get('/', async (req, res) => {
  try {
    ...
    if (userId && userPurchases[userId]) {
      prompt += `Focus on recommendations for user: ${userId} (who bought: ${userPurchases[userId].join(', ')})\n`;
    }
    prompt += 'Return a JSON array of objects: [{"name":..., "reason":...}] where reason explains why each product is recommended.';
    ...
    const all = [...trending, ...aiProducts].filter(
      (v, i, arr) => v && arr.findIndex((p) => p._id.toString() === v._id.toString()) === i
    );
